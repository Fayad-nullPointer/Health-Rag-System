...
from rag.rag_pipeline import rag_pipeline_async
...
def log_event(event_type: str, data: dict):
    logger.info(
        json.dumps({"event": event_type, **data}, ensure_ascii=False, default=str)
    )
...
engine = intent_classifier.IntentChatbotEngine(groq_client=client, intents=INTENTS)
...
async def detect_language(message: str):
    return await asyncio.to_thread(language_model.predict, message)


async def detect_emotion(message: str):
    return await asyncio.to_thread(emotion_model.predict, message)
...
async def process_message(
    ...
    log_event("input", {"trace_id": trace_id, "message_length": len(message)})
    ...
    language_result, emotion_result = await asyncio.gather(
        detect_language(message), detect_emotion(message)
    )

    language = str(language_result)
    emotion = emotion_result["emotion"]

    log_event("language_detection", {"trace_id": trace_id, "language": language})

    log_event("emotion_detection", {"trace_id": trace_id, "emotion": emotion})
    ...
    intent_data = await engine.classify_intent_async(
        message, language, emotion, chat_history=chat_history
    )
    ...
    log_event("intent_classification", {"trace_id": trace_id, **intent_data})
    ...
    if intent == "self_harm_intent":
        ...
        log_event("user_country", {"trace_id": trace_id, "country": country})

        hotline = get_hotline(country)

        log_event(
            "crisis_hotline_selected",
            {
                "trace_id": trace_id,
        ...
        response = await handle_self_harm_async(
            user_message=message, language=language, hotline=hotline
        )
