...
The backend application is fully containerized using Docker with an optimized multi-stage build.

### Services

| Service | Technology | Purpose |
|---------|------------|---------|
| API Server | FastAPI | Backend REST API |
| Vector Database | Qdrant | Stores and retrieves embeddings |
| Database | SQLite | User and chat history persistence |
...
```
docker pull alihashish09/health-rag-backend:optimized
```

---

### 3. Update Docker Compose Image

In `docker-compose.yml`:

```
app:  image: alihashish09/health-rag-backend:optimized
```

The Qdrant image is pulled automatically:
...
```
health-rag-apphealth-rag-qdrant
```
...
The following Docker volumes preserve data:

|Volume|Purpose|
|---|---|
|`qdrant_storage`|Vector database data|
|`qdrant_snapshots`|Qdrant snapshots|
|`huggingface_cache`|Cached ML models|

Data remains available after container restart.
